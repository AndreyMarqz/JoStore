...
export function ProductPage({
  ...
  return (
    <section className="product-page" aria-labelledby="product-page-title">
      <button type="button" className="product-page-back" onClick={onBack}>
      ...
      <div className="product-page-layout">
          ...
          >
            {images.map((image, index) => (
              <SwiperSlide key={`${product.id}-${index}`}>
                <div className={`product-page-media ${product.accent}`}>
                  {image ? (
                    <img
                      src={image}
        ...
        <div className="product-page-details">
          <p className="product-page-label">Produto selecionado</p>
          <h1 id="product-page-title">{product.name}</h1>
          <p className="product-page-description">{product.description}</p>
          <p className="product-page-price">{product.price}</p>
          <p className="product-page-shipping">{product.shipping}</p>
            ...
            <div className="product-page-size-options">
              {sizes.map((size) => (
                <button
                  key={size}
                  type="button"
                  className={`product-page-size${selectedSize === size ? " is-selected" : ""}`}
          ...
          <div className="product-page-actions">
            <button
              type="button"
              className="product-page-action product-page-action-secondary"
              onClick={onAddToCart}
            >
            ...
            <button
              type="button"
              className="product-page-action product-page-action-primary"
              onClick={onBuyNow}
            >
              Comprar agora
            </button>
          ...
          <p className="product-page-assurance">
            Compra segura · Troca facilitada · Entrega acompanhada
          </p>
        </div>
      </div>
    </section>
